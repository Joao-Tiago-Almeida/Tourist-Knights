...
void possible_moves(Tabuleiro *tabuleiro){
    ...
    //  Para cada ponto, verifico se o próximo ponto do caminho é válido
    for(int i = 0; i < ((PasseioTipoB*)tabuleiro->passeio)->num_pontos - 1; i++)
    {
        ...
        for(int j = 0; j < MOVES; j++){
            ...
            //  Caso a coordenada xx da próxima casa seja válida
            ...
            if(movement[j].x == ((PasseioTipoB*)tabuleiro->passeio)->pontos[i+1].x){
                ...
                //  Caso a coordenada yy da próxima casa seja válida
